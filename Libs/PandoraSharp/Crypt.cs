...
namespace PandoraSharp
{
    public class Crypto
    {
        public static Blowfish InKey = new Blowfish(Const.CryptInKey);
        public static Blowfish OutKey = new Blowfish(Const.CryptOutKey);

        internal static long DecryptSyncTime(string s)
        {
            string b = InKey.Decrypt(s, false);
            try
            {
            ...
            {
                Util.Log.O("Failed to parse sync time '" + b + "': " + e + "\r\nRaw:" + s);
                throw;
            }
        }
        ...
        public class Blowfish
        {
            ...
            public Blowfish(string key) : this(System.Text.Encoding.ASCII.GetBytes(key)) {}
            ...
            public Blowfish(byte[] key)
            {
                ...
                for (i = 0; i < N + 2; ++i)
                {
                    ...
                    System.Diagnostics.Debug.Assert(_p != null, "P != null");
                    _p[i] = _p[i] ^ data;
                }
                ...
                for (i = 0; i < 4; ++i)
                {
                    for (j = 0; j < 256; j += 2)
                    {
                        ...
                        System.Diagnostics.Debug.Assert(_s != null, "S != null");
                        _s[i, j] = datal;
                        _s[i, j + 1] = datar;
                    }
                }
            ...
            public void Encrypt(ref byte[] data)
            {
                if (data.Length%8 != 0)
                {
                    ...
                    System.Buffer.BlockCopy(data, 0, data2, 0, data.Length);
            ...
            public string Decrypt(string p, bool breakOnNull = true)
            {
                ...
                for (int i = 0; i < p.Length; i += 2)
                    bytes[i/2] = System.Convert.ToByte(string.Concat(p[i], p[i + 1]), 16);

                Decrypt(ref bytes);

                System.Text.StringBuilder sb = new System.Text.StringBuilder();

                foreach (byte b in Enumerable.TakeWhile(bytes, b => b != 0 || !breakOnNull)) {
                    sb.Append((char) b);
                }
